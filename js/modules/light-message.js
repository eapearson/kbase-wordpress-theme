		...
		*/
		...
		addTarget: {
		    value: function (targetConfig) {
		    	// console.log('adding target with id '+targetConfig.id+', filter='+JSON.stringify(targetConfig.filter));
				if (targetConfig.id) {		    	
		...
		*/
		...
		processMessage: {
			...
			if (handler) {
			    if (msg.broadcast) {
					var msgq = this;
					// console.log('processing '+handler.listeners.length + ' listeners');
					for (var i=0; i < handler.listeners.length; i++) {
					    var target = handler.listeners[i];
					    ...
					    if (target.filter && pass) {
							...
							// console.log(msg.id+'>'+i+':pass? '+pass+'filter: '+JSON.stringify(target.filter)+', from: '+msg.from+', to: '+msg.to);
					    }
					    if (pass) {
							try {
							    targetObject[target.method](msg);
							} catch(e) {
							    msgq.handleError(e, msg, i, target.method);
							} finally {
					...
					if (!target) {
					    //console.log("Message target '"+targetId+"' identified by string was not found: "+JSON.stringify(handler.targets));
					    return;	
					}
					var	targetObject = target.object;
					//} else {
					...
					if (targetObject) {
					    try {
							...
							this.handleError(e, msg, targetId, target.method);
					    } finally {
		...
		*/
		...
		on: {
		    value: function(messageId, config) {
				...
				if (config) {
				    if (!config.method) {						
						config.method = "on"+messageId;
				    }
				} else {
					throw 'Missing message handler configuration'
				}
